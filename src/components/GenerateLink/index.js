...
function GenerateLink() {
  ...
  const URL = `https://araipar.github.io?to=${encodeURIComponent(name).replace(/%20/g, "+")}`;
        ...
        <div className="col-md-10 col-md-offset-1" style={{ marginTop: '28px' }}>
          <div class="table-responsive">
            <table class="table">
              ...
              <tbody>
                {data.map((d, index) => {
                  const mapURL = `https://araipar.github.io?to=${encodeURIComponent(d.name).replace(/%20/g, "+")}&type=invitation&code=${d.code}`;
                  return (
                    <tr>
                      <td>{index + 1}</td>
                      <td>{d.name}</td>
